...
public class SubmissionSchemaMapper implements Mapper<Submission, SubmissionSchema> {
  ...
  public SubmissionSchema mapTo(Submission submission) {
    ...
    for (SubmissionTestcase testcase : submission.getSubmissionTestcases()) {
      com.codebox.shared_dtos.schema.SubmissionTestcase testcaseRecord = new com.codebox.shared_dtos.schema.SubmissionTestcase();
  ...
  public Submission mapFrom(SubmissionSchema submissionSchema) {
    ...
    submission.setId(submissionSchema.getId()
        .toString());
    submission.setCode(submissionSchema.getCode()
        .toString());
    submission.setLanguage(submissionSchema.getLanguage()
        .toString());
    ...
    submission.setStatus(submissionSchema.getStatus()
        .toString());

    // Map nested SubmissionTestcases
    List<SubmissionTestcase> testcases = new ArrayList<>();
    for (com.codebox.shared_dtos.schema.SubmissionTestcase testcaseRecord : submissionSchema.getSubmissionTestcases()) {
      SubmissionTestcase testcase = new SubmissionTestcase();
      testcase.setInput(testcaseRecord.getInput()
          .toString());
      testcase.setOutput(testcaseRecord.getOutput()
          .toString());
      testcase.setExpectedOutput(testcaseRecord.getExpectedOutput()
          .toString());
      testcase.setType(testcaseRecord.getType()
          .toString());
      testcase.setStatus(testcaseRecord.getStatus()
          .toString());
